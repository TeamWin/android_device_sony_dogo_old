...
BOOTREC_DEVICE := $(LOCAL_PATH)/root/etc/bootrec-device
TARGET_PREBUILTS := device/sony/dogo/prebuilts


INSTALLED_BOOTIMAGE_TARGET := $(PRODUCT_OUT)/boot.img
$(INSTALLED_BOOTIMAGE_TARGET): $(PRODUCT_OUT)/kernel $(uncompressed_ramdisk) $(recovery_uncompressed_ramdisk) $(INSTALLED_RAMDISK_TARGET) $(INITSH) $(MKBOOTIMG) $(MINIGZIP) $(INTERNAL_BOOTIMAGE_FILES)
	$(call pretty,"Boot image: $@")

	$(hide) rm -fr $(PRODUCT_OUT)/combinedroot
	$(hide) mkdir -p $(PRODUCT_OUT)/combinedroot/sbin

	$(hide) cp $(TARGET_PREBUILTS)/logo.rle $(PRODUCT_OUT)/combinedroot/logo.rle
	...
	$(hide) cp $(TARGET_PREBUILTS)/busybox $(PRODUCT_OUT)/combinedroot/sbin/
	$(hide) cp $(TARGET_PREBUILTS)/extract_elf_ramdisk $(PRODUCT_OUT)/combinedroot/sbin/
	$(hide) cp $(TARGET_PREBUILTS)/keycheck $(PRODUCT_OUT)/combinedroot/sbin/
